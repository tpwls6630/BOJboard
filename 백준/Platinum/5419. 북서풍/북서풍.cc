...
int main() {
	...
	while (T--) {
		...
		vector<pii> island(n);
		FOR(i, n) {
			cin >> island[i].first >> island[i].second; island[i].second *= -1;
		}
		...
		sort(all(island), secondComp);

		ll ans = 0;
		for (auto e : island) {
			ans += seg.query(1, e.first);
			seg.update(e.first, seg.get(e.first) + 1);
		}
